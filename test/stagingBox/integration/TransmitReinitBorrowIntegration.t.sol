...
contract TransmitReinitBorrowIntegration is SBIntegrationSetup {
    ...
    function testTransmitReinitIntegrationBorrowEmitsBorrow(uint256 fuzzPrice)
        public
    {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
        ...
        uint256 sbStableBalanceBefore = s_stableToken.balanceOf(
            address(s_deployedSB)
        );
        ...
        uint256 expectedZ = (sbSafeTrancheBalanceBefore * s_ratios[2]) /
            s_ratios[0];
    ...
    function testTransmitReinitIntegrationBorrowTransfersSafeTrancheFromSBToCBB(
        uint256 fuzzPrice
    ) public {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
        ...
        assertEq(
            cbbSafeTrancheBalanceBefore + sbSafeTrancheBalanceBefore,
            cbbSafeTrancheBalanceAfter
        );
    }

    function testTransmitReinitIntegrationBorrowTransfersRiskTrancheFromSBToCBB(
        uint256 fuzzPrice
    ) public {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
    ...
    function testTransmitReinitIntegrationBorrowMintsSafeSlipsToSB(
        uint256 fuzzPrice
    ) public {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
        ...
        assertEq(
            sbSafeSlipBalanceBefore + sbSafeTrancheBalanceBefore,
            sbSafeSlipBalanceAfter
        );
    }

    function testTransmitReinitIntegrationBorrowMintsRiskSlipsToSB(
        uint256 fuzzPrice
    ) public {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
    ...
    function testTransmitReinitIntegrationBorrowDoesNotChangeSBStableBalance(
        uint256 fuzzPrice
    ) public {
        setupStagingBox(fuzzPrice);
        setupTranches(false, address(s_deployedSB), s_deployedCBBAddress);
