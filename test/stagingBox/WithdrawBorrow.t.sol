...
contract WithdrawBorrow is SBIntegrationSetup {
    ...
    function WithdrawBorrowMints() private {
        ...
        s_safeTranche.mint(address(s_deployedSB), (s_maxMint / s_deployedSB.riskRatio()) * s_deployedSB.safeRatio());
        vm.stopPrank();
        ...
        ISlip(s_deployedSB.s_borrowSlipTokenAddress()).mint(s_user, s_maxMint);
        vm.stopPrank();
    }

    function testTransfersSafeTrancheFromStagingBoxToMsgSender(uint256 _fuzzPrice, uint256 _borrowSlipAmount) public {
        ...
        uint256 msgSenderSafeTrancheBalanceBeforeWithdraw = ITranche(s_deployedConvertibleBondBox.safeTranche()).balanceOf(s_user);

        _borrowSlipAmount = bound(_borrowSlipAmount, 1, sbSafeTrancheBalanceBeforeWithdraw);
        ...
        uint256 msgSenderSafeTrancheBalanceAfterWithdraw = ITranche(s_deployedConvertibleBondBox.safeTranche()).balanceOf(s_user);

        assertEq(sbSafeTrancheBalanceBeforeWithdraw - _borrowSlipAmount, sbSafeTrancheBalanceAfterWithdraw);
        assertEq(msgSenderSafeTrancheBalanceBeforeWithdraw + _borrowSlipAmount, msgSenderSafeTrancheBalanceAfterWithdraw);
        assertFalse(_borrowSlipAmount == 0);
    }

    function testTransfersRiskTrancheFromStagingBoxToMsgSender(uint256 _fuzzPrice, uint256 _borrowSlipAmount) public {
        setupStagingBox(_fuzzPrice);
        setupTranches(true, s_user, address(s_deployedSB));
        WithdrawBorrowMints();
        ...
        uint256 msgSenderRiskTrancheBalanceBeforeWithdraw = ITranche(s_deployedConvertibleBondBox.riskTranche()).balanceOf(s_user);
        ...
        uint256 msgSenderRiskTrancheBalanceAfterWithdraw = ITranche(s_deployedConvertibleBondBox.riskTranche()).balanceOf(s_user);
    ...
    function testEmitsBorrowWithdrawal(uint256 _fuzzPrice, uint256 _borrowSlipAmount) public {
        setupStagingBox(_fuzzPrice);
        setupTranches(true, s_user, address(s_deployedSB));
        WithdrawBorrowMints();
