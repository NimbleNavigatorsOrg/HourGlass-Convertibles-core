...
contract RedeemSafeSlipsForTranchesAndUnwrap is
    RedeemLendSlipsForStablesTestSetup
{
    function testRedeemSafeSlipsForTranchesAndUnwrapBurnsMsgSenderLendSlips(
        uint256 _fuzzPrice,
        uint256 _lendAmount,
        uint256 _timeWarp,
        ...
        uint256 safeSlipAmount = ISlip(s_deployedSB.safeSlipAddress())
            .balanceOf(s_lender);

        vm.assume(safeSlipAmount > 400);

        vm.startPrank(s_lender);
        ISlip(s_deployedSB.safeSlipAddress()).approve(
        ...
        StagingLoanRouter(s_stagingLoanRouter)
            .redeemSafeSlipsForTranchesAndUnwrap(s_deployedSB, safeSlipAmount);
    ...
    function testRedeemSafeSlipsForTranchesBurnsSafeSlipsFromMsgSender(
        uint256 _fuzzPrice,
        uint256 _lendAmount,
        uint256 _timeWarp,
        uint256 _lendSlipAmount
        ...
        uint256 lenderSafeSlipBalanceBefore = ISlip(
            s_deployedSB.safeSlipAddress()
        ).balanceOf(address(s_lender));

        uint256 safeSlipAmount = IERC20(s_deployedSB.safeSlipAddress())
            .balanceOf(s_lender);
        ...
        uint256 cbbSafeSlipBalanceAfter = ISlip(s_deployedSB.safeSlipAddress())
            .balanceOf(address(s_deployedConvertibleBondBox));
        uint256 lenderSafeSlipBalanceAfter = ISlip(
            s_deployedSB.safeSlipAddress()
        ).balanceOf(address(s_lender));
        ...
        assertEq(
            lenderSafeSlipBalanceBefore - buttonAmount,
            lenderSafeSlipBalanceAfter
        );
    ...
    function testRedeemSafeSlipsForTranchesReturnsUnderlyingToMsgSender(
        uint256 _fuzzPrice,
        uint256 _lendAmount,
        uint256 _timeWarp,
        uint256 _lendSlipAmount
