...
contract RedeemLendSlipsForStables is RedeemLendSlipsForStablesTestSetup {
    function testRedeemLendSlipsForStablesTransfersUnderlyingFromMsgSender(
        uint256 _fuzzPrice,
        uint256 _lendAmount,
        ...
        uint256 lendSlipBalance = ISlip(s_deployedSB.lendSlip()).balanceOf(
            s_lender
        );

        vm.startPrank(s_lender);
        ISlip(s_deployedSB.lendSlip()).approve(
            address(s_stagingLoanRouter),
            type(uint256).max
        );
        vm.stopPrank();

        uint256 stableAmount = s_stagingBoxLens.viewRedeemLendSlipsForStables(
            s_deployedSB,
            lendSlipBalance
        );
        ...
        if (cbbStableBalance >= stableAmount) {
            ...
            uint256 maxRedeemableLendSlips = squareRootOf(
                cbbStableBalance *
